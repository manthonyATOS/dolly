...
cmd="deepspeed {} \
     ...
     --warmup-steps 50 \
     --test-size 200 \
     --lr 5e-6".format(num_gpus_flag,input_model,deepspeed_config,local_output_dir,dbfs_output_dir)
print(cmd)

cpu_proc=cmd.replace("deepspeed --num_gpus=0","").replace("--module training.trainer", "training\\trainer.py").replace("deepspeed","")
print("*****************************************")
print(cpu_proc)
#os.system(cmd)
exit(0)
